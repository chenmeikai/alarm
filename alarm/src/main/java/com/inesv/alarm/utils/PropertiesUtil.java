    ...
    synchronized static private void loadProps(){
        ...
        try {
           //第一种,通过类加载器进行获取properties文件流
            in = PropertiesUtil.class.getClassLoader().getResourceAsStream("alarm.properties");
           //第二种,通过类进行获取properties文件流-->
            //in = PropertyUtil.class.getResourceAsStream("/alarm.properties");
            props.load(in);
        } catch (FileNotFoundException e) {
            logger.error("alarm.properties文件未找到");
        } catch (IOException e) {
            ...
            try {
                ...
                logger.error("alarm.properties文件流关闭出现异常");
            }
        }
    }
